...
Batch-loads data/nodes.csv and data/edges.csv into Memgraph using the graph
model:

    (:User {id})
    (:User)-[:VOTED_FOR]->(:User)

Memgraph speaks the Bolt protocol and Cypher, so it is loaded using the
official Neo4j Python driver, which is Memgraph's recommended Python client.
...
CREATE_INDEX_QUERY = "CREATE INDEX ON :User(id)"
...
def main():
    ...
    print("Memgraph import complete")
    return metrics
